...
在本次项目中，我们继续完善 state 的定义，有一种在做特征工程的感觉。

最终训练出来的 policy，牺牲了一部分稳定性，增加了主动进攻的概率，场面变得稍稍好看一点点，但 player 死亡的概率也随之增加.
...
state-8 指的是 player 的架势条被打崩，此时硬直了，boss 一般会接下劈危，此时应该向后垫步吧。
...
state-10 是指 player posture 从高点下降了一些，有可能 boss 的攻势彻底停止了，如果此时 player 是防御状态，或许可以尝试攻击。为了避免 player 发起攻击的时候，boss也同时发起攻击，我们额外计算了相邻两帧的图像相似度，如果图像过于不相似，说明 boss 可能正在发起攻击。当然，如果图像过于相似，也可能是boss正在射箭。从最终效果来看，并不好，会错过一些攻击的机会，也有偶尔出现误攻击的情况。

弦一郎其实是个 I 人，他的攻击欲望并不强: 如果 player 不主动发起攻击，boss 一般只会低频率的普通攻击，偶尔会使用前摇特别长非常容易应对的擒拿危，在这个过程中，player 几乎不会受伤，只会缓缓积累架势条；一旦 player 的攻击频率变高，boss 怒气值上来之后，它的攻击频率也会增加，而且也会更多的使用一些比较罕见的招式来阴人， 比如下段危和原地突刺危甚至还有变种，新人遇到这种情况难免会手忙脚乱。分类模型也不能正确识别这两个危，所以 player 受伤的概率也大大增加，受伤之后的处理做的并不好，所以死亡概率也会相应增加。
...
state 有点过拟合了，过度的拟合了弦一郎的招式，几乎没有泛化能力，碰到其它的 boss 是打不下去的。 当然了，人类玩家打完弦一郎再去打其它 boss，同样也是泛化不了的，也需要重新拟合新 boss 的招式。
