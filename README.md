...
最终训练出来的 policy，牺牲了一部分稳定性，增加了主动进攻的概率，场面变得稍稍好看一点点，偶尔也能做到忍杀 boss, 但 player 死亡的概率也随之增加.
...
弦一郎的个性是“遇弱则弱，遇强则强”: 如果 player 不主动攻击，boss 一般是用低频率的普通攻击来挑逗 player，偶尔会使用前摇特别长非常容易应对的擒拿危，在这个过程中，player 几乎不会受伤，只会缓缓积累架势条；
一旦 player 的攻击频率变高，boss 就有可能被惹毛，它的攻击频率也会增加，而且也会更多的使用一些比较罕见的招式， 比如原地突刺危和下段危。分类模型不能正确识别这两个危，所以 player 受伤的概率也大大增加，受伤之后的处理做的并不好，所以死亡概率也会相应增加。
